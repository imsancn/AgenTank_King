...
var CLOAK_HOLD_THREAT = 5;         // 隐身中: 星在敌火力线+敌<=此距离 -> 不追星, 转向敌人预瞄(避免踏入贴脸秒杀区)
// --- 传送抢星 / 草丛埋伏调参 ---
...
var LEAD_CAP = 11;                // 提前量上限(帧); 覆盖中距压上敌
...
// === P0.3D: Teleport 脱离来袭弹（修复沙盒验证到的「撞弹 crash」败局）===
  ...
  // === P0.3S: 主动 Teleport 抢星（修复头号败因「星战0收集」）===
  // 实证: 绳师48号 frame1 teleport 到星旁抢首星, 哪吒全程走路→0星 vs 5星惨败。
  // 触发: teleport 就绪 + 星 BFS 距离>4(走路太慢) + 我到星不比敌远(+3容差) + 帧<400(非残局拖延)。
  // 落点: 星 BFS 1~2 步邻格 + 距敌 manhattan>=3(防 freeze 秒杀) + 合法非墙非弹。
  // 仅在此条件组合下介入; 不满足时完全不触发 → 对基线零行为改变。
  {
    var _tpsSkill = safeSkill(me);
    var _tpsReady = _tpsSkill && _tpsSkill.remainingCooldownFrames === 0
                    && (_tpsSkill.type === 'teleport' || typeof me.teleport === 'function');
    var _tpsCdOk = frame - lastTeleportFrame >= TELEPORT_COOLDOWN_FRAMES;
    if (_tpsReady && _tpsCdOk && star && Array.isArray(star) && star.length >= 2
        && typeof star[0] === 'number' && frame < FRAME_TIMEOUT_WARNING) {
      var _stx = star[0], _sty = star[1];
      var _tSf = bfsFrom(_stx, _sty, map);
      var _tMk = mx * 1000 + my;
      if (_tSf && _tSf.dist[_tMk] !== undefined) {
        var _td2s = _tSf.dist[_tMk];
        var _tEk = enemyPos ? (enemyPos[0] * 1000 + enemyPos[1]) : -1;
        var _te2s = (enemyPos && _tSf.dist[_tEk] !== undefined) ? _tSf.dist[_tEk] : 999;
        // 星须够远(走路>4帧才值得传送)且我可竞争(不比敌远超3步)
        if (_td2s > 4 && _td2s <= _te2s + 3 && _td2s <= 14) {
          var _tLand = null;
          var _tMw = mapWidth(map), _tMh = mapHeight(map);
          // 优先: 星 BFS 1~2 步 + 距敌>=3(防 freeze+射秒杀) + 非敌同行列(防落地被射)
          for (var _tx = 0; _tx < _tMw && !_tLand; _tx++) {
            for (var _ty = 0; _ty < _tMh && !_tLand; _ty++) {
              var _tk = _tx * 1000 + _ty;
              var _tdk = _tSf.dist[_tk];
              if (_tdk === undefined || _tdk < 1 || _tdk > 2) continue;
              if (!teleportTargetValid(_tx, _ty, enemyPos, eBullet, map)) continue;
              if (enemyPos) {
                var _tman = Math.abs(_tx - enemyPos[0]) + Math.abs(_ty - enemyPos[1]);
                if (_tman < 3) continue;
                if (_tx === enemyPos[0] || _ty === enemyPos[1]) continue;
              }
              _tLand = [_tx, _ty];
            }
          }
          // 放宽: 允许距敌>=2 + 可共线
          if (!_tLand) {
            for (var _tx2 = 0; _tx2 < _tMw && !_tLand; _tx2++) {
              for (var _ty2 = 0; _ty2 < _tMh && !_tLand; _ty2++) {
                var _tk2 = _tx2 * 1000 + _ty2;
                var _tdk2 = _tSf.dist[_tk2];
                if (_tdk2 === undefined || _tdk2 < 1 || _tdk2 > 2) continue;
                if (!teleportTargetValid(_tx2, _ty2, enemyPos, eBullet, map)) continue;
                if (enemyPos) {
                  var _tman2 = Math.abs(_tx2 - enemyPos[0]) + Math.abs(_ty2 - enemyPos[1]);
                  if (_tman2 < 2) continue;
                }
                _tLand = [_tx2, _ty2];
              }
            }
          ...
          if (_tLand) {
            try { me.teleport(_tLand[0], _tLand[1]); lastTeleportFrame = frame; stuckCounter = 0; oscHistory = []; oscStreak = 0; return; } catch (e) {}
          }
        }
      }
    }
  ...
  // === 开火线脱离（背刺 dodgeBulletV2 的中距盲区, 置于 dodge 之前）===
  // 实证(本批 6 连败共性): 全部 crashes=1 且均为"被敌弹命中"; 败局事件流显示哪吒与敌对齐(同行/列)后
  // 未垂直脱离弹道、反而原地左右甩头 → 被敌弹沿该轴命中(如 mat_BYqu30 哪吒 f14 停[12,9], 敌 f15 同列开火,
  // 哪吒 f15-18 左右甩头不动, f18 中弹 crash)。dodgeBulletV2 仅对 6 帧内命中稳触发, 中距(敌 6~12 格外开火)
  // 其 approaching 窗受时序错位/垂直邻格受阻影响常漏触发或退化为 reverse-facing(仍留在弹道), 致未脱离。
  // 修复: 直接读敌弹自身轨迹——若敌弹与哪吒共行/列且朝哪吒方向行进(必过哪吒所在格), 立即垂直脱离一步。
  //   不依赖命中帧窗/敌面朝, 稳健覆盖中距盲区; 仅在明确弹道威胁且弹近端(<12格, 远端可能撞墙)时介入,
  //   脱离后不再共线→不会持续触发(无振荡); 与 dodgeBulletV2/P0.3D/转向 hysteresis 不冲突(仅作背刺、置于其前)。
  //   不改删任何已验证模块(BFS momentum/P2.5/P0.3T/P0.3D/decideStarMove 等)。
  if (eBullet && eBullet.position && Array.isArray(eBullet.position) && eBullet.direction) {
    var _bpx = eBullet.position[0], _bpy = eBullet.position[1];
    var _bdx = 0, _bdy = 0;
    if (eBullet.direction === "up") _bdy = -1; else if (eBullet.direction === "down") _bdy = 1;
    else if (eBullet.direction === "left") _bdx = -1; else if (eBullet.direction === "right") _bdx = 1;
    var _onLine = false;
    if (_bdy === 0 && _bpy === my) {                 // 水平弹: 沿常数 y=_bpy; 与哪吒同行且朝哪吒 x 方向
      if ((_bdx > 0 && mx > _bpx) || (_bdx < 0 && mx < _bpx)) _onLine = true;
    } else if (_bdx === 0 && _bpx === mx) {          // 竖直弹: 沿常数 x=_bpx; 与哪吒同列且朝哪吒 y 方向
      if ((_bdy > 0 && my > _bpy) || (_bdy < 0 && my < _bpy)) _onLine = true;
    }
    if (_onLine) {
      var _bman = Math.abs(mx - _bpx) + Math.abs(my - _bpy);
      if (_bman <= 12) {
        var _pd = (_bdy === 0) ? ["up", "down"] : ["left", "right"];
        var _best = pickBetterSide(mx, my, _pd, enemyPos ? enemyPos[0] : 0, enemyPos ? enemyPos[1] : 0, map);
        if (_best) {
          if (dir === _best) { me.go(); return true; }
          turnTo(me, dir, _best); return true;
        }
      }
    }
  }
  ...
  //       ③低血受压。前提: 我不会因此放弃必胜射击窗口(iCanFireNow 时不触发)
  {
    ...
    var _isCloakTank = mySkillD && mySkillD.type === 'cloak';
    if (cloakReadyD && cloakCdOkD) {
      ...
      // alignedAndSafe: 仅面向敌+可立即开火时才视为"安全不闪"; 原alignedAndSafe不查朝向→背对敌时P0.5/P0b均不行动=活靶
      var alignedAndSafe = enemyVisible && (mx === enemyPos[0] || my === enemyPos[1])
                            && isLineClear(mx, my, enemyPos[0], enemyPos[1], map)
                            && aimDirCheck(mx, my, enemyPos[0], enemyPos[1]) === dir;
      // 战略隐身抢星: 正在争夺中的星(我到星不比敌远)+敌在威胁范围内→隐身安全收星
// (敌人看不到我无法瞄准; 激活闲置技能。开心果靠技能抢星制胜)
      ...
      // (覆盖 v37.1 "!alignedAndSafe" 强制开火假设: 敌近身能 stun/射时, "我能开火"不成立——会被先手控杀)。
      ...
      if (starCloakUrgent && alignedAndSafe && iCanFireNow) {
        // 已对齐可开火 → 不要隐身, 交给 P0b 开火
        starCloakUrgent = false;
      }
      var _p05Trigger = starCloakUrgent
          || (!iCanFireNow && !alignedAndSafe && (eBullet
      ...
      if (_p05Trigger) {
        ...
        // 非 cloak 坦克(teleport等): 垂直脱离敌火力线, 替代无效 me.cloak()
        // 实证: vs 李尔/misuzu teleport 抢星后落敌同轴, me.cloak() 空操作→原地转→被射杀
        if (enemyOnMyAxis && enemyVisible && enemyPos) {
          var _p05perp = (enemyPos[0] === mx) ? ['left', 'right'] : ['up', 'down'];
          var _p05best = pickBetterSide(mx, my, _p05perp, enemyPos[0], enemyPos[1], map);
          if (_p05best) {
            if (dir === _p05best) me.go(); else turnTo(me, dir, _p05best);
            return;
          }
        }
      }
  ...
  // === P0.9 近身保命开火: 敌贴脸(≤2格)且对齐射线清 → 立刻转向开火, 优先级高于星/走位 ===
// 针对败局共性: 贴脸被 freeze 锁后全程 0 还击被点杀(shotsFired=0, 被冻互撞)。
  // 复用 P3d 同款 turnTo+fire 机制; 门控严格(enemyVisible+enemyDist<=2+对齐+射线清+未开火锁+无弹在飞):
  // 敌远时根本不触发 → 对 nova/azure/crimson 基线零改变, 仅实战贴脸场景生效。
  if (enemyVisible && enemyPos && !me.bullet && !safeStatus(me).fireLocked && enemyDist <= 2) {
    if ((mx === enemyPos[0] || my === enemyPos[1]) && isLineClear(mx, my, enemyPos[0], enemyPos[1], map)) {
      var _pbDir = aimDirCheck(mx, my, enemyPos[0], enemyPos[1]);
      if (_pbDir) {
        if (dir === _pbDir) { me.fire(); lastFireFrame = frame; return; }
        else { turnTo(me, dir, _pbDir); return; }
      }
    }
  }
...
//       否则曾因 lastDodgeFrame 2 帧冷却压制了救命闪避, 被关键命中。
...
// 否则会陷入"永不射击"僵局(实测首版对 crimson 0 开火 -> 超时负)
...
// [修复] 但"敌人正垂直横移下射击线"时, 子弹飞行途中敌已离轴 -> 必空且常撞墙(shot_wall 浪费全部火力,
  //   实战 vb NewBiew 等借此一枪反杀)。故: 敌正垂直离轴(_perp)且距离>近身阈值时, 不盲喷, 交回上层 reposi 找真实射击位
  //   (trySmartFireV2 的 moveOpts 会朝有 LOS 的射击格机动); 仅近身(flight<=LEAD_CLOSE_DIST)仍原地喷保输出不被动。
  var _perp = vel.has && !vel.teleported && (
    (alignedH && vel.vy !== 0 && vel.vx === 0) ||
    (alignedV && vel.vx !== 0 && vel.vy === 0)
  );
  if (_perp && flight > 8) { // 原LEAD_CLOSE_DIST(3)太保守→4/5败局0开火; 中距仍打, 宁撞墙不做活靶
    return false;  // 敌横移离轴, 远距盲喷必空/撞墙 → 让出本帧, 上层重新机动找射击位
  }
...
// 中心偏向权重 0.3->1.5: 原 0.3 被 countOpenSpace 压制导致沿边漫游掉主动权; 真实对手在中心区交火, 预压中心更易获得对齐
...
// 关键修正: 原 P0.3D 用"以敌为镜"的落点, 竖直弹时仍落在子弹所在列 -> 仍中弹; 此处强制脱离子弹当前列/行。
